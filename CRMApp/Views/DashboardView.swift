...
// ROI 核心看板 — 月/年 ROI + 可折叠漏斗 + 画像统计（iOS 15 兼容）
...
// MARK: - 画像卡片（通用）iOS 15 兼容：Color.blue 不用 .gradient
            ...
            ForEach(profiles, id: \.name) { item in
                VStack(spacing: 4) {
                    HStack {
                        ...
                        Spacer()
                        Text("\(item.count) 人").font(.caption).foregroundColor(.secondary)
                        Text(item.pct).font(.subheadline.bold()).foregroundColor(.blue)
